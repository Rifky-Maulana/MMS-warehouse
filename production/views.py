...
from core.access import allowed_locations, production_required

from .forms import ProductionMovementForm
from .models import ProductionCategory, ProductionItem, ProductionMovement
from .services import apply_production_movement
...
def dashboard(request):
    items = _scope_items(request, ProductionItem.objects.filter(is_active=True))
    movements = _scope_movements(request, ProductionMovement.objects.all())
...
def item_list(request):
    ...
    items = _scope_items(request, ProductionItem.objects.select_related("category", "location")).order_by("name")
...
def movement_list(request):
    ...
    qs = _scope_movements(request, ProductionMovement.objects.select_related("item", "item__location", "user", "supplier")).order_by("-created_at")
    ...
    return render(request, "production/movement_list.html", {
...
def movement_create(request):
    if request.method == "POST":
        form = ProductionMovementForm(request.POST, user=request.user)
        if form.is_valid():
            movement = form.save(commit=False)
            movement.user = request.user
            try:
                apply_production_movement(movement)
            except DjangoValidationError as e:
                form.add_error(None, "; ".join(e.messages))
            else:
                messages.success(request, "Transaksi berhasil dicatat.")
                return redirect("production:movement_create")
    else:
        form = ProductionMovementForm(user=request.user)
    return render(request, "production/movement_form.html", {"form": form})
...
def analytics(request):
    ...
    base = _scope_movements(request, ProductionMovement.objects.filter(
    ...
    scoped_all = _scope_movements(request, ProductionMovement.objects.all())
    User = get_user_model()
    pencatat_list = User.objects.filter(
        id__in=scoped_all.values_list("user_id", flat=True).distinct()
    ).order_by("username")

    return render(request, "production/analytics.html", {
        "date_from": date_from, "date_to": date_to,
        "total_in": total_in, "total_out": total_out,
        "chart_data": {"keluar": top("out"), "masuk": top("in"), "trend": trend},
        "categories": ProductionCategory.objects.all(),
