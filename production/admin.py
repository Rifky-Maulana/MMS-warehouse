...
class ProductionMovementForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if mtype in (ProductionMovement.Type.IN, ProductionMovement.Type.OUT) and qty == 0:
            raise forms.ValidationError("Jumlah untuk barang masuk/keluar harus lebih dari 0.")
        if mtype == ProductionMovement.Type.OUT and item and qty:
...
class ProductionMovementAdmin(admin.ModelAdmin):
    form = ProductionMovementForm
